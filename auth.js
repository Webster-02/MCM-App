...
export async function requireAuth(allowedRoles = []) { const roles = Array.isArray(allowedRoles) ? allowedRoles : [allowedRoles]; if (TEST_MODE) { const stored = getStoredUser(); if (stored?.role && roles.length && !roles.includes(stored.role)) { redirectForRole(stored.role); return null; } const role = stored?.role || roles[0]; if (!role || !TEST_USERS[role]) throw new Error('No test role configured for this page.'); return setStoredUser({ ...TEST_USERS[role] }); } const firebaseUser = auth.currentUser; if (!firebaseUser) { window.location.assign('login.html'); return null; } const profile = await firebaseProfile(firebaseUser); if (!profile) throw new Error('User profile not found.'); if (roles.length && !roles.includes(profile.role)) { redirectForRole(profile.role); return null; } return setStoredUser(profile); }
export async function loginUser(email, password, role = '') { if (TEST_MODE) { const selected = TEST_USERS[role] || Object.values(TEST_USERS).find(user => user.email === String(email || '').trim()); if (!selected) throw new Error('Select a valid demo role.'); return setStoredUser({ ...selected }); } const credential = await signInWithEmailAndPassword(auth, String(email).trim(), password); const profile = await firebaseProfile(credential.user); if (!profile) throw new Error('User profile not found.'); return setStoredUser(profile); }
export async function registerUser(email, password, profile = {}) { if (TEST_MODE) throw new Error('Registration is disabled in demo mode.'); const credential = await createUserWithEmailAndPassword(auth, String(email).trim(), password); const newProfile = normalizeProfile(credential.user.uid, { ...profile, email: credential.user.email }); await setDoc(doc(db, 'users', credential.user.uid), newProfile); return setStoredUser(newProfile); }
...
if (typeof window !== 'undefined') {
  ...
  const dashboardRoles = { 'student-dashboard.html': 'student', 'class-management.html': 'admin', 'svl-dashboard.html': 'svl' };
  const activeRole = dashboardRoles[page];
  if (activeRole) {
    const bootShell = () => {
      import('./js/dashboard-shell.js').then(({ initDashboardShell }) => initDashboardShell({ allowedRoles: [activeRole], activeRole })).catch((error) => console.warn('Dashboard shell failed to load:', error));
      import('./js/mobile-navigation.js').catch((error) => console.warn('Mobile navigation failed to load:', error));
    };
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', bootShell, { once: true }); else bootShell();
  }
}
